...
use crate::components::store::{self as s, Entity, EntityKey, EntityOp, Value};
...
impl EntityCache {
    ...
    pub fn with_current(
        ...
        current: LfuCache<s::EntityKey, Option<s::Entity>>,
    ) -> EntityCache {
        EntityCache {
            current,
            updates: HashMap::new(),
    ...
    pub fn get(&mut self, key: &s::EntityKey) -> Result<Option<s::Entity>, s::QueryExecutionError> {
    ...
    pub fn remove(&mut self, key: s::EntityKey) {
        self.entity_op(key, s::EntityOp::Remove);
    }
    ...
    pub fn append(&mut self, operations: Vec<s::EntityOperation>) {
        assert!(!self.in_handler);

        for operation in operations {
            match operation {
                s::EntityOperation::Set { key, data } => {
                    self.entity_op(key, EntityOp::Update(data));
                }
                s::EntityOperation::Remove { key } => {
    ...
    pub fn as_modifications(mut self) -> Result<ModificationsAndCache, s::QueryExecutionError> {
        ...
        let mut missing_by_subgraph: BTreeMap<_, BTreeMap<&s::EntityType, Vec<&str>>> =
            BTreeMap::new();
        for key in missing {
            missing_by_subgraph
