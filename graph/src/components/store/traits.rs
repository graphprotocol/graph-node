...
use crate::components::store as s;
...
pub trait SubscriptionManager: Send + Sync + 'static {
    ...
    fn subscribe_no_payload(&self, entities: BTreeSet<SubscriptionFilter>) -> s::UnitStream;
}
...
pub trait SubgraphStore: Send + Sync + 'static {
    ...
    ) -> Result<s::DeploymentLocator, StoreError>;
    ...
    fn reassign_subgraph(
        &self,
        deployment: &s::DeploymentLocator,
        node_id: &NodeId,
    ) -> Result<(), StoreError>;
    ...
    fn assignments(&self, node: &NodeId) -> Result<Vec<s::DeploymentLocator>, StoreError>;
    ...
    fn locators(&self, hash: &str) -> Result<Vec<s::DeploymentLocator>, StoreError>;
}
...
pub trait WritableStore: Send + Sync + 'static {
    ...
    async fn fail_subgraph(&self, error: s::SubgraphError) -> Result<(), StoreError>;
    ...
    fn transact_block_operations(
        ...
        deterministic_errors: Vec<s::SubgraphError>,
    ) -> Result<(), StoreError>;
    ...
    fn get_many(
        &self,
        ids_for_type: BTreeMap<&s::EntityType, Vec<&str>>,
    ) -> Result<BTreeMap<s::EntityType, Vec<Entity>>, StoreError>;
    ...
    async fn load_dynamic_data_sources(
        &self,
    ) -> Result<Vec<s::StoredDynamicDataSource>, StoreError>;

    /// Report the name of the shard in which the subgraph is stored. This
    /// should only be used for reporting and monitoring
    fn shard(&self) -> &str;

    async fn health(&self, id: &DeploymentHash) -> Result<s::SubgraphHealth, StoreError>;
...
pub trait ChainStore: Send + Sync + 'static {
    ...
    async fn upsert_block(&self, block: Arc<dyn s::Block>) -> Result<(), Error>;

    fn upsert_light_blocks(&self, blocks: &[&dyn s::Block]) -> Result<(), Error>;
    ...
    async fn set_chain_head(
        self: Arc<Self>,
        block: Arc<dyn s::Block>,
        cursor: String,
    ) -> Result<(), Error>;

    /// Returns the blocks present in the store.
    fn blocks(&self, hashes: &[s::H256]) -> Result<Vec<serde_json::Value>, Error>;
    ...
    fn block_hashes_by_block_number(&self, number: BlockNumber) -> Result<Vec<s::H256>, Error>;

    /// Confirm that block number `number` has hash `hash` and that the store
    /// may purge any other blocks with that number
    fn confirm_block_hash(&self, number: BlockNumber, hash: &s::H256) -> Result<usize, Error>;
...
pub trait QueryStore: Send + Sync {
    ...
    fn block_number(&self, block_hash: s::H256) -> Result<Option<BlockNumber>, StoreError>;
...
pub trait StatusStore: Send + Sync + 'static {
    ...
    fn version_info(&self, version_id: &str) -> Result<s::VersionInfo, StoreError>;
