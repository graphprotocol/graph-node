...
impl AscHeap for StoreContextMut<'_, WasmInstanceContext> {
    fn raw_new(&mut self, bytes: &[u8], gas: &GasCounter) -> Result<u32, DeterministicHostError> {
        let ctx = self.data_mut();
        ctx.asc_heap.unwrap().raw_new(bytes, gas)
    }
    ...
    fn read<'a>(
        ...
        let ctx = self.data();
        ctx.asc_heap.unwrap().read(offset, buffer, gas)
    }

    fn read_u32(&self, offset: u32, gas: &GasCounter) -> Result<u32, DeterministicHostError> {
        let ctx = self.data();
        ctx.asc_heap.unwrap().read_u32(offset, gas)
    }

    fn api_version(&self) -> semver::Version {
        let ctx = self.data();
        ctx.asc_heap.unwrap().api_version()
    }

    fn asc_type_id(
        &self,
        type_id_index: crate::runtime::IndexForAscTypeId,
    ) -> Result<u32, HostExportError> {
        let ctx = self.data();
        ctx.asc_heap.unwrap().asc_type_id(type_id_index)
    }
}
...
impl WasmInstanceContext {
    ...
    fn store_get_scoped(
        ...
        let host_exports = self.ctx.host_exports.cheap_clone();
        let _timer = self
            .host_metrics
            .cheap_clone()
        ...
        let entity_type: String = asc_get(&read_store, self.asc_heap_mut(), entity_ptr, gas)?;
        let id: String = asc_get(&read_store, self.asc_heap_mut(), id_ptr, gas)?;
        ...
        let host_metrics = self.host_metrics.cheap_clone();
        let debug_fork = self.ctx.debug_fork.cheap_clone();
                ...
                Some(fork) => {
                    ...
                    match entity_option {
                        Some(entity) => {
                            ...
                            let entity =
                                asc_new(store, self.asc_heap_mut(), &entity.sorted(), gas)?;
...
impl WasmInstanceContext {
    ...
    pub fn store_set(
        ...
        let stopwatch = self.host_metrics.stopwatch.cheap_clone();
        let logger = self.ctx.logger.cheap_clone();
        let block_number = self.ctx.block_ptr.block_number();
        stopwatch.start_section("host_export_store_set__wasm_instance_context_store_set");

        let read_store = store.as_context();
        let entity: String = asc_get(&read_store, self.asc_heap_mut(), entity_ptr, gas)?;
        let id: String = asc_get(&read_store, self.asc_heap_mut(), id_ptr, gas)?;
        let data = asc_get(&read_store, self.asc_heap_mut(), data_ptr, gas)?;
    ...
    pub fn store_load_related(
        ...
        let read_store = store.as_context();
        let entity_type: String = asc_get(&read_store, self.asc_heap_mut(), entity_type_ptr, gas)?;
        let id: String = asc_get(&read_store, self.asc_heap_mut(), id_ptr, gas)?;
        let field: String = asc_get(&read_store, self.asc_heap_mut(), field_ptr, gas)?;
        let host_exports = self.ctx.host_exports.cheap_clone();
        ...
        let ret = asc_new(store, self.asc_heap_mut(), &entities, gas)?;
        Ok(ret)
    }
    ...
    pub fn bytes_to_string(
        ...
        let string = host_exports.bytes_to_string(
            &self.ctx.logger,
            asc_get(&store.as_context(), self.asc_heap_ref(), bytes_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &string, gas)
    }
    ...
    pub fn bytes_to_hex(
        ...
        let bytes: Vec<u8> = asc_get(&store.as_context(), self.asc_heap_ref(), bytes_ptr, gas)?;
        ...
        asc_new(store, self.asc_heap_mut(), &hex, gas)
    }
    ...
    pub fn big_int_to_string(
        ...
        let n: BigInt = asc_get(&store.as_context(), self.asc_heap_ref(), big_int_ptr, gas)?;
        gas.consume_host_fn_with_metrics(
            gas::DEFAULT_GAS_OP.with_args(gas::complexity::Mul, (&n, &n)),
            "big_int_to_string",
        )?;
        asc_new(store, self.asc_heap_mut(), &n.to_string(), gas)
    }
    ...
    pub fn big_int_from_string(
        ...
        let result = host_exports.big_int_from_string(
            asc_get(&store.as_context(), self.asc_heap_ref(), string_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn string_to_h160(
        ...
        let s: String = asc_get(&store.as_context(), self.asc_heap_ref(), str_ptr, gas)?;
        let h160 = self.ctx.host_exports.string_to_h160(&s, gas)?;
        asc_new(store, self.asc_heap_mut(), &h160, gas)
    }
    ...
    pub fn json_try_from_bytes(
        ...
        let read_store = store.as_context();
        let bytes: Vec<u8> = asc_get(&read_store, self.asc_heap_mut(), bytes_ptr, gas)?;
        let host_exports = self.ctx.host_exports.cheap_clone();
    ...
    pub fn ipfs_cat(
        ...
        let read_store = store.as_context();
        let link = asc_get(&read_store, self.asc_heap_mut(), link_ptr, gas)?;
        let host_exports = self.ctx.host_exports.cheap_clone();
        let logger = self.ctx.logger.cheap_clone();
        let ipfs_res = host_exports.ipfs_cat(&logger, link);
        let logger = self.ctx.logger.cheap_clone();
        match ipfs_res {
            Ok(bytes) => asc_new(store, self.asc_heap_mut(), &*bytes, gas).map_err(Into::into),

            // Return null in case of error.
            Err(e) => {
                info!(&logger, "Failed ipfs.cat, returning `null`";
                                    "link" => asc_get::<String, _, _>(&read_store, self.asc_heap_mut(), link_ptr, gas)?,
                                    "error" => e.to_string());
                Ok(AscPtr::null())
            }
        }
    ...
    pub fn ipfs_get_block(
        ...
        let link = asc_get(&read_store, self.asc_heap_mut(), link_ptr, gas)?;
        let host_exports = self.ctx.host_exports.cheap_clone();
        let ipfs_res = host_exports.ipfs_get_block(&self.ctx.logger, link);
        match ipfs_res {
            Ok(bytes) => asc_new(store, self.asc_heap_mut(), &*bytes, gas).map_err(Into::into),
    ...
    pub fn ipfs_map(
        ...
        let read_store = store.as_context();
        let link: String = asc_get(&read_store, self.asc_heap_mut(), link_ptr, gas)?;
        let callback: String = asc_get(&read_store, self.asc_heap_mut(), callback, gas)?;
        let user_data: store::Value = asc_get(&read_store, self.asc_heap_mut(), user_data, gas)?;

        let flags = asc_get(&read_store, self.asc_heap_mut(), flags, gas)?;
        ...
        let output_states = HostExports::ipfs_map(
            &self.ctx.host_exports.link_resolver.cheap_clone(),
            store,
            link.clone(),
            &callback,
            user_data,
    ...
    pub fn json_to_i64(
        ...
        host_exports.json_to_i64(
            asc_get(&read_store, self.asc_heap_mut(), json_ptr, gas)?,
            gas,
        )
    }
    ...
    pub fn json_to_big_int(
        ...
        asc_new(store, self.asc_heap_mut(), &*big_int, gas)
    }
    ...
    pub fn crypto_keccak_256(
        ...
        let input = host_exports.crypto_keccak_256(
            asc_get(&read_store, self.asc_heap_mut(), input_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), input.as_ref(), gas)
    }
    ...
    pub fn big_int_plus(
        ...
        let read_store = store.as_context();
        let host_exports = self.ctx.host_exports.cheap_clone();
        let x = asc_get(&read_store, self.asc_heap_mut(), x_ptr, gas)?;
        let y = asc_get(&read_store, self.asc_heap_mut(), y_ptr, gas)?;
    ...
    pub fn big_int_divided_by(
        ...
        let x = asc_get(&read_store, self.asc_heap_mut(), x_ptr, gas)?;
        let y = asc_get(&read_store, self.asc_heap_mut(), y_ptr, gas)?;
        let result = self.ctx.host_exports.big_int_divided_by(x, y, gas)?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn big_int_divided_by_decimal(
        ...
        let read_store = store.as_context();
        let x = BigDecimal::new(asc_get(&read_store, self.asc_heap_mut(), x_ptr, gas)?, 0);
        let host_exports = self.ctx.host_exports.cheap_clone();
        let result = host_exports.big_decimal_divided_by(
            x,
            asc_get(&read_store, self.asc_heap_mut(), y_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn big_int_pow(
        ...
        let result = host_exports.big_int_pow(
            asc_get(&read_store, self.asc_heap_mut(), x_ptr, gas)?,
            exp,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    ...
    pub fn bytes_to_base58(
        ...
        let result = host_exports.bytes_to_base58(
            asc_get(&read_store, self.asc_heap_mut(), bytes_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn big_decimal_to_string(
        ...
        let result = host_exports.big_decimal_to_string(
            asc_get(&read_store, self.asc_heap_mut(), big_decimal_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn big_decimal_from_string(
        ...
        let result = host_exports.big_decimal_from_string(
            asc_get(&read_store, self.asc_heap_mut(), string_ptr, gas)?,
            gas,
        )?;
        asc_new(store, self.asc_heap_mut(), &result, gas)
    }
    ...
    pub fn data_source_create(
        ...
        let read_store = store.as_context();
        let logger = self.ctx.logger.cheap_clone();
        let block_number = self.ctx.block_ptr.number;
        let name: String = asc_get(&read_store, self.asc_heap_mut(), name_ptr, gas)?;
        let params: Vec<String> = asc_get(&read_store, self.asc_heap_mut(), params_ptr, gas)?;
        let host_exports = self.ctx.host_exports.cheap_clone();
    ...
    pub fn data_source_create_with_context(
        ...
        let logger = self.ctx.logger.cheap_clone();
        let block_number = self.ctx.block_ptr.number;
        let name: String = asc_get(&store.as_context(), self.asc_heap_mut(), name_ptr, gas)?;
        let params: Vec<String> =
            asc_get(&store.as_context(), self.asc_heap_mut(), params_ptr, gas)?;
        let context: HashMap<_, _> =
            asc_get(&store.as_context(), self.asc_heap_mut(), context_ptr, gas)?;
    ...
    pub fn data_source_address(
        ...
        let addr = self.ctx.host_exports.data_source_address(gas)?;
        asc_new(store, self.asc_heap_mut(), addr.as_slice(), gas)
    }
    ...
    pub fn data_source_network(
        ...
        asc_new(
            store,
            self.asc_heap_mut(),
            &self.ctx.host_exports.data_source_network(gas)?,
            gas,
        )
    }
    ...
    pub fn data_source_context(
        ...
        asc_new(store, self.asc_heap_mut(), &ds_ctx, gas)
    }
    ...
    pub fn ens_name_by_hash(
        ...
        let read_store = store.as_context();
        let hash: String = asc_get(&read_store, self.asc_heap_mut(), hash_ptr, gas)?;
        ...
        name.map(|name| asc_new(store, self.asc_heap_mut(), &*name, gas).map_err(Into::into))
    ...
    pub fn log_log(
        ...
        let msg: String = asc_get(&store.as_context(), self.asc_heap_mut(), msg, gas)?;
        self.ctx
            .host_exports
    ...
    pub fn ethereum_encode(
        ...
        let data = self.ctx.host_exports.ethereum_encode(
            asc_get(&store.as_context(), self.asc_heap_ref(), token_ptr, gas)?,
            gas,
        );

        // return `null` if it fails
        data.map(|bytes| asc_new(store, self.asc_heap_mut(), &*bytes, gas))
            .unwrap_or(Ok(AscPtr::null()))
    }
    ...
    pub fn ethereum_decode(
        ...
        let read_store = store.as_context();
        let types = asc_get(&read_store, self.asc_heap_mut(), types_ptr, gas)?;
        let data = asc_get(&read_store, self.asc_heap_mut(), data_ptr, gas)?;
        let result = self.ctx.host_exports.ethereum_decode(types, data, gas);

        // return `null` if it fails
        result
            .map(|param| asc_new(store, self.asc_heap_mut(), &param, gas))
            .unwrap_or(Ok(AscPtr::null()))
    }
