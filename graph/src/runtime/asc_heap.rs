...
pub async fn asc_new_or_missing<H, O, A>(
    heap: &mut H,
    object: &Option<O>,
    gas: &GasCounter,
    type_name: &str,
...
where
    H: AscHeap + Send + ?Sized,
    O: ToAscObj<A>,
    A: AscType + AscIndexId,
{
    match object {
        Some(o) => asc_new(heap, o, gas).await,
...
pub async fn asc_new_or_null<H, O, A>(
    heap: &mut H,
    object: &Option<O>,
    gas: &GasCounter,
) -> Result<AscPtr<A>, HostExportError>
...
#[async_trait]
impl<C: AscType, T: ToAscObj<C> + Sync> ToAscObj<C> for &T {
    async fn to_asc_obj<H: AscHeap + Send + ?Sized>(
        &self,
        heap: &mut H,
        gas: &GasCounter,
    ) -> Result<C, HostExportError> {
        (*self).to_asc_obj(heap, gas).await
    }
}
