    ...
    C: Blockchain,
{
    ...
    /// The BlockStream is reconciling the subgraph store state with the chain
    /// store state.
    ///
    /// Valid next states: YieldingBlocks, Idle, BeginReconciliation (in case of
    /// revert)
    Reconciliation(Pin<Box<dyn Future<Output = Result<NextBlocks<C>, Error>> + Send>>),

    /// The BlockStream is emitting blocks that must be processed in order to
    /// bring the subgraph store up to date with the chain store.
    ///
    /// Valid next states: BeginReconciliation
    YieldingBlocks(Box<VecDeque<BlockWithTriggers<C>>>),

    /// The BlockStream experienced an error and is pausing before attempting to
    /// produce blocks again.
    ///
    /// Valid next states: BeginReconciliation
    RetryAfterDelay(Pin<Box<dyn Future<Output = Result<(), Error>> + Send>>),

    /// The BlockStream has reconciled the subgraph store and chain store
    /// states. No more work is needed until a chain head update.
    ///
    /// Valid next states: BeginReconciliation
    Idle,
}

/// A single next step to take in reconciling the state of the subgraph store
/// with the state of the chain store.
enum ReconciliationStep<C>
where
    C: Blockchain,
{
    /// Revert(to) the block the subgraph should be reverted to, so it becomes
    /// the new subgraph head.
    Revert(BlockPtr),

    /// Move forwards, processing one or more blocks. Second element is the
    /// block range size.
    ...
    C: Blockchain,
{
    ...
    // The payload is block the subgraph should be reverted to, so it becomes the new subgraph
    // head.
    Revert(BlockPtr),
    Done,
}
...
where
    C: Blockchain,
{
    /// Perform reconciliation steps until there are blocks to yield or we are
    /// up-to-date.
    ...
    /// Determine the next reconciliation step. Does not modify Store or
    /// ChainStore.
    async fn get_next_step(&self) -> Result<ReconciliationStep<C>, Error> {
        ...
        // subgraph_ptr > head_ptr shouldn't happen, but if it does, it's safest to just
        // stop.
        ...
        //
        // We will use a different approach to deciding the step direction depending on
        // how far the subgraph ptr is behind the head ptr.
        //
        // Normally, we need to worry about chain reorganizations -- situations where
        // the Ethereum client discovers a new longer chain of blocks different
        // from the one we had processed so far, forcing us to rollback one or
        // more blocks we had already processed.
        // We can't assume that blocks we receive are permanent.
        //
        // However, as a block receives more and more confirmations, eventually it
        // becomes safe to assume that that block will be permanent.
        // The probability of a block being "uncled" approaches zero as more and more
        // blocks are chained on after that block.
        ...
        //
        // Accordingly, if the subgraph ptr is really far behind the head ptr, then we
        // can trust that the Ethereum node knows what the real, permanent block
        // is for that block number.
        ...
        // reorg_threshold blocks behind), then we have to allow for the possibility
        // that the block might be on the main chain now, but might become
        // uncled in the future.
        //
        // Most importantly: Our ability to make this assumption (or not) will determine
        // what Ethereum RPC calls can give us accurate data without race
        // conditions. (This is mostly due to some unfortunate API design
        // decisions on the Ethereum side)
        if subgraph_ptr.is_none()
            || (head_ptr.number - subgraph_ptr.as_ref().unwrap().number) > reorg_threshold
        {
            // Since we are beyond the reorg threshold, the Ethereum node knows what block
            // has been permanently assigned this block number.
            // This allows us to ask the node: does subgraph_ptr point to a block that was
            // permanently accepted into the main chain, or does it point to a block that
            // was uncled?
            ...
            // - Scan 1 block: 0 triggers found, max_range_size = 10, range_size = 10
            // - Scan 10 blocks: 2 triggers found, 0.2 per block, range_size = 1000 / 0.2 =
            //   5000
            // - Scan 5000 blocks: 10000 triggers found, 2 per block, range_size = 1000 / 2
            //   = 500
            // - Scan 500 blocks: 1000 triggers found, 2 per block, range_size = 1000 / 2 =
            //   500
            ...
            // the block associated with a particular block number on the Ethereum node
            // could change under our feet at any time.
            //
            ...
            // To determine the step direction, we need to find out if the subgraph ptr
            // refers to a block that is an ancestor of the head block.
            // We can do so by walking back up the chain from the head block to the
            // appropriate block number, and checking to see if the block we
            // found matches the subgraph_ptr.
            ...
            // In principle this block should be in the store, but we have seen this error
            // for deep reorgs in ropsten.
                    ...
                    match next_blocks_future.poll_unpin(cx) {
                        Poll::Ready(Ok(next_block_step)) => match next_block_step {
                            NextBlocks::Blocks(next_blocks, block_range_size) => {
                                // We had only one error, so we infer that reducing the range size
                                // is what fixed it. Reduce the max
                                // range size to prevent future errors.
