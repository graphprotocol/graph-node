...
/// Client for connecting to Amp core and executing SQL queries.
pub trait Client {
...
/// Metadata sent to the Amp server with the SQL query.
#[derive(Debug, Clone)]
...
/// Represents a batch response resulting from query execution on the Amp server.
#[derive(Debug, Clone)]
pub enum ResponseBatch {
    /// Contains the batch data received from the Amp server.
    Batch { data: RecordBatch },

    /// Contains the reorg message received from the Amp server.
    ///
