...
pub type IndexNodeServiceResponse = DynTryFuture<'static, Response<String>, GraphQLServerError>;
    ...
    S: Store,
{
    ...
    fn serve_file(contents: &'static str, content_type: &'static str) -> Response<String> {
        Response::builder()
    ...
    fn handle_graphiql() -> Response<String> {
    ...
    fn handle_graphql_options(_request: Request<Incoming>) -> Response<String> {
        Response::builder()
            .status(200)
            .header(ACCESS_CONTROL_ALLOW_ORIGIN, "*")
    ...
    fn handle_temp_redirect(destination: &str) -> Result<Response<String>, GraphQLServerError> {
    ...
    /// Handles 404s.
    pub(crate) fn handle_not_found() -> Response<String> {
        Response::builder()
...
impl<Q, S> Service<Request<Incoming>> for IndexNodeService<Q, S>
where
    Q: GraphQlRunner,
    S: Store,
{
                ...
                .map(move |result| match result {
                    ...
                    Err(err @ GraphQLServerError::ClientError(_)) => {
                        ...
                        Ok(Response::builder()
                            ...
                            .header(CONTENT_TYPE, "text/plain")
                            .header(ACCESS_CONTROL_ALLOW_ORIGIN, "*")
                            .body(format!("Invalid request: {}", err).to_string())
                            .unwrap())
                    }
                    Err(err @ GraphQLServerError::QueryError(_)) => {
                        ...
                        Ok(Response::builder()
                            ...
                            .header(CONTENT_TYPE, "text/plain")
                            .header(ACCESS_CONTROL_ALLOW_ORIGIN, "*")
                            .body(format!("Query error: {}", err).to_string())
                            .unwrap())
                    }
