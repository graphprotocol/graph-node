...
pub struct GraphQLResponse {
    result: Result<Arc<QueryResult>, GraphQLServerError>,
}

impl GraphQLResponse {
    /// Creates a new GraphQLResponse future based on the result generated by
    /// running a query.
    pub fn new(result: Result<Arc<QueryResult>, GraphQLServerError>) -> Self {
...
mod tests {
    ...
    fn generates_200_for_query_results() {
        ...
        let future = GraphQLResponse::new(Ok(Arc::new(query_result)));
        let response = future.wait().expect("Should generate a response");
    ...
    fn canonical_serialization() {
        macro_rules! assert_resp {
            ($exp: expr, $obj: expr) => {{
                ...
                let res = Arc::new(QueryResult::new(Some($obj)));
