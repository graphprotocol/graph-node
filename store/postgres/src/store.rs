...
use graph::prelude::{
    ...
    trace, warn, web3, AttributeIndexDefinition, BigInt, BlockNumber, BlockPointer,
    ChainHeadUpdateListener as _, ChainHeadUpdateStream, ChainStore, Entity, EntityKey,
    EntityModification, EntityOrder, EntityQuery, EntityRange, Error, EthereumBlock,
    EthereumBlockPointer, EthereumCallCache, EventProducer as _, Future, Future01CompatExt,
    LightEthereumBlock, Logger, MetadataOperation, MetricsRegistry, QueryExecutionError, Schema,
    Sink as _, StopwatchMetrics, StoreError, StoreEvent, StoreEventStream, StoreEventStreamBox,
    Stream, SubgraphAssignmentProviderError, SubgraphDeploymentId, SubgraphDeploymentStore,
...
impl ChainStore for Store {
    fn initialize_chain(
        &self,
        new_version: &String,
        new_genesis_block: &BlockPointer,
    ) -> Result<(), Error> {
        ...
        match network_identifiers_opt {
            ...
            None => {
                insert_into(ethereum_networks)
                    .values((
                        ...
                        net_version.eq::<Option<String>>(Some(new_version.to_owned())),
                        genesis_block_hash
                            .eq::<Option<String>>(Some(new_genesis_block.hash.as_hex_string())),
                    ))
                    .on_conflict(name)
                    .do_nothing()
            ...
            Some((Some(old_version), Some(old_genesis_block_hash))) => {
                ...
                if old_genesis_block_hash != new_genesis_block.hash.as_hex_string() {
                    panic!(
                        "Ethereum node provided genesis block hash `{}`, \
                         but we expected `{}`. Did you change networks \
                         without changing the network name?",
                        new_genesis_block.hash.as_hex_string(),
                        old_genesis_block_hash,
                    );
                }
            }
