...
impl SubgraphStoreInner {
    ...
    fn create_deployment_internal(
        ...
        pconn.transaction(|conn| -> Result<_, StoreError> {
            ...
            let changes = primary::Connection::create_subgraph_version(
                conn,
                name,
                &site,
                node_id,
            ...
            primary::Connection::send_store_event(conn, &self.sender, &event)?;
            Ok(())
        })?;
        Ok(site.as_ref().into())
    }
    ...
    pub fn copy_deployment(
        ...
        pconn.transaction(|conn| -> Result<_, StoreError> {
            ...
            // the copy
            let changes = primary::Connection::assign_subgraph(conn, dst.as_ref(), &node)?;
            let event = StoreEvent::new(changes);
            primary::Connection::send_store_event(conn, &self.sender, &event)?;
            Ok(())
        })?;
    ...
    pub(crate) fn send_store_event(&self, event: &StoreEvent) -> Result<(), StoreError> {
        let mut conn = self.primary_conn()?;
        conn.transaction(|conn| primary::Connection::send_store_event(conn, &self.sender, event))
    }
...
impl SubgraphStoreTrait for SubgraphStore {
    ...
    fn create_subgraph(&self, name: SubgraphName) -> Result<String, StoreError> {
        let mut pconn = self.primary_conn()?;
        primary::Connection::create_subgraph(&mut pconn.conn, &name)
    }

    fn remove_subgraph(&self, name: SubgraphName) -> Result<(), StoreError> {
        let mut pconn = self.primary_conn()?;
        pconn.transaction(|conn| -> Result<_, StoreError> {
            let changes = primary::Connection::remove_subgraph(conn, name)?;
            primary::Connection::send_store_event(conn, &self.sender, &StoreEvent::new(changes))
        })
    }
    ...
    fn reassign_subgraph(
        ...
        let mut pconn = self.primary_conn()?;
        pconn.transaction(|conn| -> Result<_, StoreError> {
            let changes = primary::Connection::reassign_subgraph(conn, site.as_ref(), node_id)?;
            primary::Connection::send_store_event(conn, &self.sender, &StoreEvent::new(changes))
        })
    }
