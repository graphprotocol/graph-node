...
impl EntityData {
    ...
    pub fn deserialize<T: FromEntityData>(
        self,
        layout: &Layout,
            ...
            j::Object(mut map) => {
                ...
                if include_block_range {
                    let optional_block_range = map
                        .remove(BLOCK_RANGE_COLUMN)
                        .clone()
                        .map(|r| {
                            Some(T::Value::from_column_value(&ColumnType::BlockRange, r).ok()?)
                        })
                        .flatten();

                    match optional_block_range {
                        Some(block_range) => {
                            out.insert_entity_data(BLOCK_RANGE_COLUMN.to_owned(), block_range)
                        }
    ...
    pub fn deserialize_with_layout<T: FromEntityData>(
        ...
        self.deserialize(layout, false)
    }

    /// Returns entity records along with the block range of the record.
    pub fn deserialize_with_layout_and_block_range<T: FromEntityData>(
        self,
        layout: &Layout,
    ) -> Result<T, StoreError> {
        self.deserialize(layout, true)
    }
}
...
impl<'a> QueryFilter<'a> {
    ...
    fn changed_at_block(
        ...
        QueryValue(value, &ColumnType::Int).walk_ast(out.reborrow())?;
        out.push_sql(op.as_str());
        out.push_sql("lower(");
        out.push_identifier(BLOCK_RANGE_COLUMN)?;
        out.push_sql(")");
...
impl<'a> QueryFragment<Pg> for QueryFilter<'a> {
    fn walk_ast(&self, mut out: AstPass<Pg>) -> QueryResult<()> {
        ...
        match &self.filter {
            ...
            ChangedAtBlock(block_number) => {
                self.changed_at_block(&Value::Int(*block_number), c::LowerRangeEqual, out)?
            }
        }
        Ok(())
    }
