...
This guide covers everything you need to know about log storage in graph-node, including configuration, querying, and choosing the right backend for your deployment.
...
Graph Node supports storing and querying subgraph logs through multiple backends. Logs include:
...
All logs can be captured, stored, and queried through a unified GraphQL API.

**Available backends:**
- **File**: JSON Lines files on local filesystem (for development)
...
- **`id`**: Unique identifier (`<subgraph-id>-<timestamp>`)
- **`subgraphId`**: Deployment hash
- **`timestamp`**: ISO 8601 timestamp with nanosecond precision
- **`level`**: Log level (critical, error, warning, info, debug)
- **`text`**: Log message with key-value arguments
- **`arguments`**: Array of key-value pairs from structured logging
...
**Best for:** Local development, testing, low-volume deployments
...
⚠️ **Important Limitation**: File-based logs load the **entire file into memory** when querying. This makes queries slow for large log files.

**Performance characteristics:**
- Query time: O(n) where n = number of log entries
- Memory usage: Entire file loaded into RAM
- Suitable for: Files under 100MB (≈100k-500k log entries)
...
- Entire file loaded into memory for queries
- No indexing (slow searches)
- No log rotation implemented (currently)
...
- **Aggregations**: Analyze log patterns and statistics
- **Scalability**: Handles billions of log entries
- **High availability**: Supports clustering and replication
- **Kibana integration**: Rich visualization and dashboards
...
- May incur costs in cloud deployments
...
When log storage is disabled (the default), logs from subgraphs are **not persisted**. They may still be written to stdout/stderr by graph-node, but they're not stored in a queryable format.
...
- Running minimal test deployments
- Logs are not required for your use case
- Using external log collection (e.g., container logs)
...
CLI arguments provide the same functionality as environment variables and can be mixed together.
...
**Example:**
```bash
# Environment variable sets Elasticsearch
export GRAPH_LOG_STORE_BACKEND=elasticsearch
export GRAPH_LOG_STORE_ELASTICSEARCH_URL=http://localhost:9200

# CLI argument overrides to use file-based
graph-node \
  --log-store-backend file \
  --log-store-file-dir ./logs \
  ...

# Result: Uses file-based logs (CLI wins)
```
...
# Run with file-based logs
graph-node \
  --postgres-url postgresql://graph:pass@localhost/graph-node \
  --ethereum-rpc mainnet:https://mainnet.infura.io/v3/YOUR-KEY \
  --ipfs 127.0.0.1:5001 \
  --log-store-backend file \
  --log-store-file-dir ./graph-logs
```
...
graph-node \
  ...
  --log-store-elasticsearch-user elastic \
  --log-store-elasticsearch-password secret \
  --log-store-elasticsearch-index graph-node-logs
```

#### Loki (Production with Grafana)
...
graph-node \
  ...
  --ipfs 127.0.0.1:5001 \
  --log-store-backend loki \
  --log-store-loki-url http://loki.monitoring.svc:3100 \
  --log-store-loki-tenant-id graph-node-prod
```

## Querying Logs

All log backends share the same GraphQL query interface at `http://localhost:8000/graphql` (or your configured GraphQL port).

**Note**: Queries return all log types - both user-generated logs from mapping code and system-generated runtime logs (handler execution, events, warnings, etc.). Use the `text` filter to search for specific messages, or `level` to filter by severity.
...
|--------|------|-------------|
| `subgraphId` | String (required) | Deployment hash (QmXxx...) |
...
| `text` | String | Case-insensitive substring search in log messages |
| `first` | Int | Number of results to return (default: 100) |
...
This returns both user logs mentioning "handler" and system logs about handler execution.
...
curl -X POST http://localhost:8000/graphql \
  ...
  -d '{
    "query": "{ _logs(subgraphId: \"QmExample\", level: ERROR, first: 10) { timestamp level text } }"
  }'
```
...
**File-based:**
- Queries load entire file into memory
- Performance degrades with file size
- Keep files under 100MB for reasonable performance
...
- Text searches are slower (scans chunks)
- Good performance with proper label filtering

## Migrating from Deprecated Configuration

If you're using old environment variables or CLI arguments, simply rename them to the new format. The old configuration still works (with deprecation warnings).

### Environment Variable Mapping

| Old (Deprecated) | New |
|-----------------|-----|
| `GRAPH_LOG_STORE` | `GRAPH_LOG_STORE_BACKEND` |
| `GRAPH_ELASTICSEARCH_URL` | `GRAPH_LOG_STORE_ELASTICSEARCH_URL` |
| `GRAPH_ELASTICSEARCH_USER` | `GRAPH_LOG_STORE_ELASTICSEARCH_USER` |
| `GRAPH_ELASTICSEARCH_PASSWORD` | `GRAPH_LOG_STORE_ELASTICSEARCH_PASSWORD` |
| `GRAPH_ELASTIC_SEARCH_INDEX` | `GRAPH_LOG_STORE_ELASTICSEARCH_INDEX` |
| `GRAPH_LOG_LOKI_ENDPOINT` | `GRAPH_LOG_STORE_LOKI_URL` |
| `GRAPH_LOG_LOKI_TENANT` | `GRAPH_LOG_STORE_LOKI_TENANT_ID` |
| `GRAPH_LOG_FILE_DIR` | `GRAPH_LOG_STORE_FILE_DIR` |
| `GRAPH_LOG_FILE_MAX_SIZE` | `GRAPH_LOG_STORE_FILE_MAX_SIZE` |
| `GRAPH_LOG_FILE_RETENTION_DAYS` | `GRAPH_LOG_STORE_FILE_RETENTION_DAYS` |

### CLI Argument Mapping

| Old (Deprecated) | New |
|-----------------|-----|
| `--elasticsearch-url` | `--log-store-elasticsearch-url` |
| `--elasticsearch-user` | `--log-store-elasticsearch-user` |
| `--elasticsearch-password` | `--log-store-elasticsearch-password` |

### Deprecation Warnings

When using old configuration, you'll see warnings like:
```
WARN Using deprecated environment variable 'GRAPH_ELASTICSEARCH_URL', please use 'GRAPH_LOG_STORE_ELASTICSEARCH_URL' instead
```

These are informational - your configuration will continue to work. Update to the new names to remove the warnings.
...
| Production (< 10k logs/day) | **File** or **Loki** | File is simple, Loki if using Grafana |
| Production (> 10k logs/day) | **Elasticsearch** or **Loki** | Both handle scale well |
| Need advanced search | **Elasticsearch** | Full-text indexing and aggregations |
...
1. **Keep files small** - Aim for < 100MB per subgraph
2. **Use log rotation** - Manually archive/delete old files
3. **Monitor disk usage** - Files can grow quickly with verbose logging
4. **Use JSON tools** - `jq` is excellent for querying .jsonl files locally
...
- Ensure subgraph is actually logging (check mapping code)

**Problem: Queries are slow**
- File is too large (> 100MB)
- Consider archiving old logs
- Switch to Elasticsearch or Loki for better performance
...
- You're using old environment variable or CLI argument names
- See [Migration Guide](#migration-guide) to update configuration
- Not urgent - old configuration still works

**Problem: GraphQL query returns empty array**
- Verify subgraph ID is correct (deployment hash, not subgraph name)
- Check backend is configured and running
- Verify logs are being written (check backend directly)
- Check time range filter isn't excluding all logs
