...
pub trait AscHeap: Sized {
    ...
    where
        ...
        T: FromAscObj<C>,
    {
        let data = asc_ptr.read_ptr(self)?;
        Ok(T::from_asc_obj(data, self)?)
    }
...
impl AscType for bool {
    ...
    fn from_asc_bytes(asc_obj: &[u8]) -> Result<Self, DeterministicHostError> {
        if asc_obj.len() != size_of::<Self>() {
            Err(DeterministicHostError(anyhow::anyhow!("Incorrect size")))
        } else {
            Ok(asc_obj[0] != 0)
        }
    }
...
impl AscType for i16 {
    ...
    fn from_asc_bytes(asc_obj: &[u8]) -> Result<Self, DeterministicHostError> {
        if asc_obj.len() != size_of::<Self>() {
            ...
            Ok(Self::from_le_bytes([asc_obj[0], asc_obj[1]]))
        }
    }
}
    ...
    fn from_asc_bytes(asc_obj: &[u8]) -> Result<Self, DeterministicHostError> {
        if asc_obj.len() != size_of::<Self>() {
            ...
            Ok(Self::from_le_bytes([
                asc_obj[0], asc_obj[1], asc_obj[2], asc_obj[3],
            ]))
        }
    }
}
...
impl AscType for f32 {
    ...
    fn from_asc_bytes(asc_obj: &[u8]) -> Result<Self, DeterministicHostError> {
        Ok(Self::from_bits(u32::from_asc_bytes(asc_obj)?))
    }
}

impl AscType for f64 {
    fn to_asc_bytes(&self) -> Result<Vec<u8>, DeterministicHostError> {
        Ok(self.to_bits().to_asc_bytes()?)
    }

    fn from_asc_bytes(asc_obj: &[u8]) -> Result<Self, DeterministicHostError> {
        Ok(Self::from_bits(u64::from_asc_bytes(asc_obj)?))
    }
}
