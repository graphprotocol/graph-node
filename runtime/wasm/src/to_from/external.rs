...
impl ToAscObj<AscEnum<EthereumValueKind>> for graph::abi::DynSolValue {
    fn to_asc_obj<H: AscHeap + ?Sized>(
        &self,
        heap: &mut H,
        gas: &GasCounter,
...
impl FromAscObj<AscEnum<EthereumValueKind>> for graph::abi::DynSolValue {
    fn from_asc_obj<H: AscHeap + ?Sized>(
        ...
        let value = match asc_enum.kind {
            ...
            EthereumValueKind::Int => {
                ...
                let x = graph::abi::I256::from_limbs(n.to_signed_u256().0);
            ...
            EthereumValueKind::Uint => {
                let ptr: AscPtr<AscBigInt> = AscPtr::from(payload);
                let n: BigInt = asc_get(heap, ptr, gas, depth)?;
                let x = graph::abi::U256::from_limbs(n.to_unsigned_u256().0);
