...
use syn::{Fields, FieldsNamed, Ident, Item, ItemEnum, ItemStruct, Token};
...
//takes optional optional list of required fields '__required__{name:TypeName}'
// and enumerations field decraration with types, i.e. sum{single:
// ModeInfoSingle,multi: ModeInfoMulti} intended use is in build.rs with
// tonic_build's type_attribute(<...>, <...>) to generate type implementation of
// graph::runtime::ToAscObj Annotation example:
...
//takes required network name attribute to form variant name
// graph::runtime::IndexForAscTypeId::<NetworkName>+<TypeName> Annotation
// example:
...
//     const INDEX_ASC_TYPE_ID: graph::runtime::IndexForAscTypeId =
// graph::runtime::IndexForAscTypeId::CosmosMyMessageType ;  }
...
// }
//
//Note: this macro makes heavy reliance on types to be available via
// crate::protobuf (network chain crate root/src/protobuf/lib.rs) please see
// usage exmple in chain::cosmos crate... lib.rs imports generates protobuf
// bindings, as well as any other needed types
...
//Example:
//  pub struct AscMyMessageTypeArray(pub
// graph_runtime_wasm::asc_abi::class::Array<graph::runtime::AscPtr<AscMyMessageType>>)
// where "AscMyMessageTypeArray" is an array type for "AscMyMessageType"
// (AscMyMessageType is generated by asc_type derive macro above) Macro, also,
// will generate code for the following 3 trait implementations
// 1. graph::runtime::ToAscObj trait
//Example:
//  impl graph::runtime::ToAscObj<AscMyMessageTypeArray> for
// Vec<AscMyMessageType> {     ...
//  }
...
//    ...
//    }
//3. graph::runtime::AscIndexId (adding expected <NetworkName><BaseType>>Array
// (CosmosMyMessageTypeArray)  variant to graph::runtime::IndexForAscTypeId is
// manual step) impl graph::runtime::AscIndexId for MyMessageTypeArray {
//    const INDEX_ASC_TYPE_ID: graph::runtime::IndexForAscTypeId =
// graph::runtime::IndexForAscTypeId::CosmosMyMessageTypeArray ;
//}
...
//     fn from_asc_bytes(asc_obj: &[u8], api_version: graph::semver::Version) ->
// Self {         assert_eq!(&asc_obj.len(), &size_of::<Self>());
//         let mut offset = 0;
//         let field_size = std::mem::size_of::<AscPtr<K>>();
//         let key =
// graph::runtime::AscType::from_asc_bytes(&asc_obj[offset..(offset +
// field_size)],         api_version.clone());
//         offset += field_size;
//         let field_size = std::mem::size_of::<AscPtr<V>>();
//         let value =
// graph::runtime::AscType::from_asc_bytes(&asc_obj[offset..(offset +
// field_size)], api_version);         offset += field_size;
//         Self { key, value }
//     }
// }
//
...
// start with offset 0
// for each field:
//     * if offset is not multiple of field alignment add padding bytes with
//       size needed to fill
...
// if end offset is not multiple of struct alignment add padding field with
// required size.
...
//     fn to_asc_bytes(&self) -> Result<Vec<u8>,
// graph::runtime::DeterministicHostError> {         let discriminant: u32 =
// match *self {             JsonValueKind::Null => 0u32,
...
//     }
//
//     fn from_asc_bytes(asc_obj: &[u8], _api_version: graph::semver::Version)
// -> Result<Self, graph::runtime::DeterministicHostError> {         let mut
// u32_bytes: [u8; size_of::<u32>()] = [0; size_of::<u32>()];         if
// std::mem::size_of_val(&u32_bytes) != std::mem::size_of_val(&asc_obj) {
//             return
// Err(graph::runtime::DeterministicHostError::from(graph::prelude::anyhow::anyhow!
...
//             _ =>
// Err(graph::runtime::DeterministicHostError::from(graph::prelude::anyhow::anyhow!
// ("value {} is out of range for {}", discr, "JsonValueKind"))),         }
//     }
// }
