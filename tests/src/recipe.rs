...
/// default `pnpm deploy:test` is used.
async fn build_subgraph(dir: &str, deploy_cmd: Option<&str>) -> DeploymentHash {
    build_subgraph_with_pnpm_cmd(dir, deploy_cmd.unwrap_or("deploy:test")).await
}

async fn build_subgraph_with_pnpm_cmd(dir: &str, pnpm_cmd: &str) -> DeploymentHash {
    build_subgraph_with_pnpm_cmd_and_arg(dir, pnpm_cmd, None).await
}

pub async fn build_subgraph_with_pnpm_cmd_and_arg(
    dir: &str,
    pnpm_cmd: &str,
    arg: Option<&str>,
) -> DeploymentHash {
