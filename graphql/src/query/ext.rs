...
use graph::prelude::{q, r, BlockNumber, Error, TryFromValue};
...
pub enum BlockConstraint {
    Hash(BlockHash),
    Number(BlockNumber),
    /// Execute the query on the latest block only if the the subgraph has
    /// progressed to or past the given block number.
    Min(BlockNumber),
    Latest,
}
