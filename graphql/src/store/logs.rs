use graph::components::log_store::{LogLevel, LogQuery};
...
fn validate_text_input(text: &str) -> Result<(), &'static str> {
    ...
    // Reject strings that look like Elasticsearch query DSL
    // This prevents injection of query syntax
    let dangerous_chars = ['{', '}', '[', ']', ':', '"'];
...
fn validate_timestamp(timestamp: &str) -> Result<(), &'static str> {
    // Basic format check: must contain 'T' separator and either 'Z' or offset
    if !timestamp.contains('T') {
        return Err("timestamp must contain 'T' separator");
    }

    if !timestamp.ends_with('Z') && !timestamp.contains('+') && !timestamp.contains('-') {
        return Err("timestamp must end with 'Z' or contain timezone offset");
    }

    // Length check to prevent extremely long inputs
    if timestamp.len() > 50 {
        return Err("timestamp too long");
    }
        ...
        match name.as_str() {
            "level" => {
                if let r::Value::Enum(level_str) = value {
                    level = Some(LogLevel::from_str(level_str).ok_or_else(|| {
