        ...
        object
            ...
            .map(|(key, value)| {
                ...
                Ok(match op {
                    ...
                    ContainsInsensitive => {
                        EntityFilter::ContainsInsensitive(field_name, store_value)
                    }
                    NotContains => EntityFilter::NotContains(field_name, store_value),
                    NotContainsInsensitive => {
                        EntityFilter::NotContainsInsensitive(field_name, store_value)
                    }
                    StartsWith => EntityFilter::StartsWith(field_name, store_value),
                    StartsWithInsensitive => {
                        EntityFilter::StartsWithInsensitive(field_name, store_value)
                    }
                    NotStartsWith => EntityFilter::NotStartsWith(field_name, store_value),
                    NotStartsWithInsensitive => {
                        EntityFilter::NotStartsWithInsensitive(field_name, store_value)
                    }
                    EndsWith => EntityFilter::EndsWith(field_name, store_value),
                    EndsWithInsensitive => {
                        EntityFilter::EndsWithInsensitive(field_name, store_value)
                    }
                    NotEndsWith => EntityFilter::NotEndsWith(field_name, store_value),
                    NotEndsWithInsensitive => {
                        EntityFilter::NotEndsWithInsensitive(field_name, store_value)
                    }
