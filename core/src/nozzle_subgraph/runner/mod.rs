...
use graph::{
    cheap_clone::CheapClone, components::store::EntityCache, data::subgraph::schema::SubgraphError,
    nozzle::Client,
};
...
async fn run_indexing<NC>(cx: &mut Context<NC>) -> Result<(), Error>
where
    NC: Client,
{
    loop {
        ...
        while let Some(result) = stream.next().await {
            ...
            if deployment_is_failed {
                if let Some(block_ptr) = cx.store.block_ptr() {
                    cx.store.unfail_non_deterministic_error(&block_ptr)?;
...
async fn run_indexing_with_retries<NC>(cx: &mut Context<NC>) -> Result<()>
where
    NC: Client,
{
    loop {
