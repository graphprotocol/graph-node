...
#[derive(Clone, Debug)]
pub struct PauseDeploymentCommand {
    pub deployment: DeploymentSelector,
}

impl PauseDeploymentCommand {
    fn try_execute(self, ctx: &GraphmanContext) -> Result<bool, GraphmanError> {
        ...
        if is_paused {
            info!(ctx.logger, "Deployment '{}' is already paused", locator);
            return Ok(true);
        }

        info!(ctx.logger, "Pausing deployment '{}'", locator);

        let changes = conn.pause_subgraph(&site)?;
        conn.send_store_event(&ctx.notification_sender, &StoreEvent::new(changes))?;

        info!(ctx.logger, "Successfully paused deployment '{}'", locator);
        Ok(true)
    }
}

impl<Ctx> GraphmanCommand<Ctx> for PauseDeploymentCommand
where
...
impl ExtensibleGraphmanCommand for PauseDeploymentCommand {}
