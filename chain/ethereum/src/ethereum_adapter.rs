            ...
            retry(format!("load block ptr {}", block_num), &logger)
                ...
                .run(move || {
                    web3.eth()
                        ...
                        .and_then(move |block| {
                            block.ok_or_else(|| {
                                anyhow::anyhow!("Ethereum node did not find block {:?}", block_num)
                            })
                        })
                })
                .from_err()
    ...
    T::Out: Send,
{
                    ...
                    .and_then(|gen_block_opt| {
                        future::result(
                            gen_block_opt
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!("Ethereum node could not find genesis block")
                                }),
                        )
                    })
            });
            ...
            net_version_future
                ...
                .map_err(|e| {
                    e.into_inner().unwrap_or_else(|| {
                        anyhow::anyhow!("Ethereum node took too long to read network identifiers")
                    })
                }),
        )
    }
            ...
            retry("eth_getBlockByNumber(latest) no txs RPC call", logger)
                ...
                .run(move || {
                    web3.eth()
                        ...
                        .map_err(|e| {
                            anyhow::anyhow!("could not get latest block from Ethereum: {}", e)
                        })
                        .from_err()
                        .and_then(|block_opt| {
                            block_opt.ok_or_else(|| {
                                anyhow::anyhow!("no latest block returned from Ethereum").into()
                            })
                        })
                })
                .map_err(move |e| {
                    e.into_inner().unwrap_or_else(move || {
                        anyhow::anyhow!("Ethereum node took too long to return latest block").into()
                    })
                }),
        )
    }
    ...
    fn uncles(
        ...
        let block_hash = match block.hash {
            Some(hash) => hash,
            None => {
                return Box::new(future::result(Err(anyhow::anyhow!(
        ...
        Box::new(
            futures::stream::futures_ordered((0..n).map(move |index| {
                ...
                retry("eth_getUncleByBlockHashAndIndex RPC call", &logger)
                    ...
                    .map_err(move |e| {
                        e.into_inner().unwrap_or_else(move || {
                            anyhow::anyhow!("Ethereum node took too long to return uncle")
                        })
                    })
            }))
            .collect(),
        ...
        Box::new(
            self.block_hash_by_block_number(&logger, chain_store, block_ptr.number, true)
                .and_then(move |block_hash_opt| {
                    block_hash_opt
                        .ok_or_else(|| {
                            anyhow::anyhow!("Ethereum node is missing block #{}", block_ptr.number)
                        })
