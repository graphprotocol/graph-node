...
use self::block_writer::BlockWriter;
use self::metrics::NetworkIndexerMetrics;
...
type LocalHeadFuture = Box<dyn Future<Item = Option<BlockPointer>, Error = Error> + Send>;
type ChainHeadFuture<N: Network> = Box<dyn Future<Item = N::Block, Error = Error> + Send>;
type OmmersFuture = Box<dyn Future<Item = Vec<dyn Block>, Error = Error> + Send>;
type BlockPointerFuture = Box<dyn Future<Item = BlockPointer, Error = Error> + Send>;
type BlockFuture = Box<dyn Future<Item = Option<dyn Block>, Error = Error> + Send>;
type BlockStream<N: Network> = Box<dyn Stream<Item = N::Block, Error = Error> + Send>;
type RevertLocalHeadFuture = Box<dyn Future<Item = BlockPointer, Error = Error> + Send>;
type AddBlockFuture = Box<dyn Future<Item = BlockPointer, Error = Error> + Send>;
...
fn load_local_head<N: Network>(context: &Context<N>) -> LocalHeadFuture {
...
fn poll_chain_head<N: Network>(context: &Context<N>) -> ChainHeadFuture<N> {
    ...
    Box::new(
        track_future!(
            ...
            context.network.clone().latest_block().from_err()
        )
...
//     block: &EthereumBlock,
// ) -> OmmersFuture {
//     let block_ptr: BlockPointer = block.into();
...
//                                     "error" => format!("{}", e),
//                                     "block" => format!("{}", BlockPointer::from(block)),
//                                 );
//
//                                 None
//                             }
...
fn fetch_blocks<N>(context: &Context<N>, block_numbers: Range<u64>) -> BlockStream<N>
where
    N: Network,
{
    ...
    Box::new(
        futures::stream::iter_ok::<_, Error>(block_numbers)
            .map(move |block_number| {
                network.clone().block_by_number(block_number)
...
fn write_block<N>(block_writer: Arc<BlockWriter>, block: N::Block) -> AddBlockFuture
where
    N: Network,
{
...
fn load_parent_block_from_store<N: Network>(
    context: &Context<N>,
    ...
    Box::new(
            ...
            future::result(
                block
                    ...
                    .and_then(|value| {
                        ...
                        Bytes::from_str(s.as_str()).map_err(|e| {
                            format_err!(
        ...
        .map(move |parent_hash: Bytes| BlockPointer::from((block_ptr.number - 1, parent_hash))),
    )
}

fn revert_local_head<N: Network>(
    context: &Context<N>,
    local_head: BlockPointer,
...
fn send_event<N>(
    event_sink: Sender<NetworkIndexerEvent<N>>,
    event: NetworkIndexerEvent<N>,
) -> SendEventFuture
where
    N: Network,
{
 ...
 * Helpers for metrics
 */
fn update_chain_and_local_head_metrics<N: Network>(
    context: &Context<N>,
    chain_head: &N::Block,
    local_head: Option<BlockPointer>,
) {
    context.metrics.chain_head.set(chain_head.number() as f64);
    context
        .metrics
        .local_head
...
#[state_machine_future(context = "Context<N: Network>")]
enum StateMachine<N: Network> {
    ...
    PollChainHead {
        local_head: Option<BlockPointer>,
        prev_chain_head: Option<BlockPointer>,
        chain_head: ChainHeadFuture<N>,
    },
...
impl<N: Network> PollStateMachine<N> for StateMachine<N> {
    fn poll_start<'a, 'c>(
        _state: &'a mut RentToOwn<'a, Start>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ...
    fn poll_load_local_head<'a, 'c>(
        ...
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterLoadLocalHead<N>, Error> {
    ...
    fn poll_poll_chain_head<'a, 'c>(
        state: &'a mut RentToOwn<'a, PollChainHead<N>>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterPollChainHead<N>, Error> {
        // Abort if the output stream has been closed.
        try_ready!(context.event_sink.poll_ready());
        ...
        match state.chain_head.poll() {
            ...
            Ok(Async::Ready(chain_head)) => {
                // TODO: Replace this with an error earlier in e.g. the EthereumAdapter.
                //
                ...
                //         "block" => format!("{}", chain_head.pointer()),
                //     );
                //
                ...
                if Some(chain_head.pointer()) != state.prev_chain_head {
                    context
                        .metrics
                ...
                debug!(
                    ...
                    "chain_head" => format!("{}", chain_head.pointer()),
                ...
                if Some(chain_head.pointer()) == state.local_head {
                    debug!(
                        context.logger,
                ...
                if chain_head.number() < state.local_head.map_or(0u64, |ptr| ptr.number) {
                    debug!(
                        context.logger,
    ...
    fn poll_process_blocks<'a, 'c>(
        state: &'a mut RentToOwn<'a, ProcessBlocks<N>>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterProcessBlocks<N>, Error> {
        // Abort if the output stream has been closed.
        try_ready!(context.event_sink.poll_ready());
    ...
    fn poll_vet_block<'a, 'c>(
        state: &'a mut RentToOwn<'a, VetBlock<N>>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterVetBlock<N>, Error> {
        // Abort if the output stream has been closed.
        try_ready!(context.event_sink.poll_ready());
    ...
    fn poll_revert_local_head<'a, 'c>(
        state: &'a mut RentToOwn<'a, RevertLocalHead<N>>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterRevertLocalHead<N>, Error> {
        // Abort if the output stream has been closed.
        try_ready!(context.event_sink.poll_ready());
    ...
    fn poll_add_block<'a, 'c>(
        state: &'a mut RentToOwn<'a, AddBlock<N>>,
        context: &'c mut RentToOwn<'c, Context<N>>,
    ) -> Poll<AfterAddBlock<N>, Error> {
        // Abort if the output stream has been closed.
        try_ready!(context.event_sink.poll_ready());
...
pub struct DefaultNetworkIndexerOptions<N>
where
    N: Network,
{
    logger: Logger,
...
pub struct DefaultNetworkIndexer<N: Network> {
    output: Option<Receiver<NetworkIndexerEvent<N>>>,
}

impl<N: Network> DefaultNetworkIndexer<N> {
    pub fn new(options: DefaultNetworkIndexerOptions<N>) -> Self
    where
        N: Network,
    {
        let DefaultNetworkIndexerOptions {
            logger,
            network,
            store,
            metrics_registry,
        ...
        let logger = logger.new(o!(
            "component" => "DefaultNetworkIndexer",
...
impl<N> EventProducer<NetworkIndexerEvent<N>> for DefaultNetworkIndexer<N>
where
    N: Network,
{
    fn take_event_stream(
        &mut self,
    ) -> Option<Box<dyn Stream<Item = NetworkIndexerEvent<N>, Error = ()> + Send>> {
        self.output.take().map(|s| {
            Box::new(s) as Box<dyn Stream<Item = NetworkIndexerEvent<N>, Error = ()> + Send>
        })
    }
}

impl<N> NetworkIndexer<N> for DefaultNetworkIndexer<N> where N: Network {}
