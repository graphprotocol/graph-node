...
fn create_subgraph(
    ...
    let manifest = SubgraphManifest {
        ...
        description: None,
        repository: None,
        schema: Schema::parse(schema, subgraph_id.clone()).expect("valid network subgraph schema"),
        data_sources: vec![],
        templates: vec![],
    };
    ...
    ops.extend(
        SubgraphDeploymentEntity::new(
            ...
            false,
            false,
            start_block.map(|ptr| EthereumBlockPointer {
                number: ptr.number,
                hash: H256::from_str(ptr.hash.hex()),
            }),
            None,
        )
