...
impl ToAscPtr for NearTrigger {
    fn to_asc_ptr<H: AscHeap>(
        ...
        Ok(match self {
            NearTrigger::Block(block) => asc_new(store, heap, block.as_ref(), gas)?.erase(),
            NearTrigger::Receipt(receipt) => asc_new(store, heap, receipt.as_ref(), gas)?.erase(),
        })
    }
}
...
//         util::mem::init_slice,
//         wasmtime::StoreContext,
//     };
//     // use graph_runtime_test::test_module_latest;

//     // #[tokio::test]
//     // async fn block_trigger_to_asc_ptr() {
//     //     let mut instance = test_module_latest("b", "boolean.wasm").await;
//     //     let trigger = NearTrigger::Block(Arc::new(block()));
//     //     let wasm_ctx = instance.store.data().clone();

//     //     let result: anyhow::Result<()> = Ok(());
//     //     let result = trigger.to_asc_ptr(
//     //         &mut instance.store.as_context_mut(),
//     //         &mut wasm_ctx.as_mut().asc_heap_mut(),
//     //         &GasCounter::new(GasMetrics::mock()),
//     //     );
//     //     assert!(result.is_ok());
//     // }
...
//     //     let mut instance = test_module_latest("b2", "boolean.wasm").await;
//     //     let wasm_ctx = instance.store.data().clone();
...
//             let init = self.read(store, offset, &mut data, gas)?;
