...
- GraphQL route to expose call cache contents by block (#3343)
- Ethereum Transaction Receipts in Event Handlers (#3373) (#3444)
- Bump max `apiVersion` to 0.0.7 and `specVersion` to 0.0.5 (#3421)
- Add Firehose blockstream metrics (#3435)
- Add `_change_block` to entities GraphQL `input` type filter (#3186)
- Add publicProofOfIndexing resolver to the indexing status API (#3358)
- Handle Ethereum Firehose decoding issues without crashing (#3483)
- Make Ethereum call revert detection more sensitive (#3490)
- New stable-hash implementation (#3485)
- Support TLS PostgreSQL connections in notification_listener (#3503)
- Arweave integration (#3465)
- Add ipfs.getBlock (#3577)
- Configure HTTP_PORT and WS_PORT via environment variables (#2832)
- Arweave: allow owner match-all filter (#3611)
- Arweave: Add sha256 pubkey support (#3613)
- Specify genesis block via env #3650
- Add partial account filters for NEAR (#3669)
- Allow limiting the number of subgraphs using a web3 provider (#3685)
- Add child filters (#3184)
- Allow to run GraphQL validations silently (#3716)
- Fast PoI (#3678)
- Select the POI algorithm (#3734)
- Add totalDifficulity from firehose block ver.2 (#3748)
- Add initial support for offchain data sources in manifest (#3743)
- Substreams trigger processor (#3763)
- Add Substreams Chain (#3764)

### Fixes

- Fix broken deployment_eth_rpc_* metrics registration for Ethereum (#3426)
- Fix check against GRAPH_CACHED_SUBGRAPH_IDS (#3436)
- Assume that immutable entities are actually new during indexing (#3451)
- Fix lookup of immutable entities (#3455)
- Fix increasing the Firehose restarts metric (#3482)
- Fix subgraph cancelation (#3481)
- Fixed BlockHandler with Call filter (#3087)
- Make conflicting_entity check work for Bytes id (#3486)
- Make traversing derived relationships work when using `Bytes` ids (#3492)
- Fix documented defaults for environment variables: Api & Spec versions (#3439)
- Avoid double-prefixing parent ids passed through as Bytes in `store` #3526
- Missing env var causing pg crash on startup (#3511)
- Run `ANALYZE` after grafting (#3538)
- Add an alias for the `ipfsOnEthereumContracts` feature (#3544)
- Rename Tendermint to Cosmos (#3543)
- Fix a problem with mixing mutable and immutable entities in interfaces (#3561)
- Handle related interface attributes with Bytes id correctly (#3567)
- Fix bug in query generation for prefixed columns and improve testing of query generation (#3579)
- Transaction gas is transaction limit in Ethereum Firehose (#3584)
- Fix blockHandler filter doc (#3593)
- Fix subgraph stacktrace (#3562)
- Use the correct parent type for type d queries (#3617)
- Handle `None`s in Cosmos (#3622)
- Fix graphql SG block check (#3610)
- Add validation for grafts created at failed block (#3627)
- Remove call.input.len() validation for Ethereum (#3651)
- Detect xdai eth_call revert (#3697)
- Fix child basis filtering on derived fields (#3702)
- Avoid large trace filter requests for Ethereum (#3733)
- Disable GraphQL validations by default (#3751)
- Fix poi cursor on deterministic error (#3749)
- Fix firehose eth CallHandler triggering on reverted calls (#3762)
- Fix default variables in start script (#3725) 
- Remove typename from POI (#3739)

### `graphman`

- Fix some graphman bugs and improve usability (#3416)
- Make sure that graphman unassign/reassign send store events (#3478)
- New graphman `fix block` command (#3270)
...
- `GRAPH_MAX_GAS_PER_HANDLER` is set to a very high value by default,
  ...
  benchmarked and refined. (#3731)
- Adjust exp gas cost (#3477)
- Alter deployment_schemas.version column to int (#3505)
- `ORDER_BY_BLOCK_RANGE` default value is now `true` (#3561)

### Misc

- Store environment variables and flags in a global struct (#3364)
- Skip 'public' chains when creating call_cache indexes (#3399)
- Refactor `SubgraphRunner` (old `InstanceManager`) - Part 2 (#3367)
- Label store_connection_error_count with shard and pool (#3415)
- Add hardware requirements to README.md (#3413)
- Remove SubgraphRunner Clones (#3420)
- Change transact block error message (#3418)
- Do not derive debug for environment variables (#3427)
- LinkResolver refactor (#3425) (#3447)
- More consistent env. vars. grouping (#3437)
- Streamline the debug output for a few types (#3445)
- Instrument provider initialization (#3422)
- Put receipts into Arcs to prevent excessive cloning. (#3443)
- Organize relational module of the `store` (#3441)
- Add docs on how to implement a new chain (#3440)
- Use reserved discriminant segments for `IndexForAscTypeId` (#3446)
- Move static filter check in Runner (#3461)
- Ignore skip block ptr if close to chain head (#3468)
- Check that the shard for 'copy create' exists (#3467)
- Add end-to-end test (#3454) (#3466)
- Rename current stable-hash implementation (#3475)
- Avoid panic if GRAPH_STORE_CONNECTION_MIN_IDLE exceeds pool_size (#3489)
- Use a much smaller initial batch size for tables with array columns (#3488)
- Correct Layout::new comment (#3491)
- Make NonNullError attestable (#3507)
- Record timing information for failed GraphQL queries (#3508)
- Add stopwatch sections to clarify bg write times (#3509)
- Some refinements on how we deal with connection pools (#3513)
- Make the log for large GraphQL results more useful (#3514)
- Represent map keys in GraphQL query results as Box<str> (#3516)
- Use the correct setting for number of LfuCache shards (#3522)
- Legacy event stream (#3506)
- Encapsulate access to the LfuCache (#3524)
- Small change to memory accounting in `LfuCache` and various changes to the cache stress testing tool (#3532)
- Remove GRAPH_ACCOUNT_TABLES setting (#3525)
- Count queries for missing subgraphs against 'notfound' (#3535)
- Log information when evicting from the query LfuCache (#3536)
- Install cmake which is required by prost in Docker image (#3539)
- Track a couple more details about LfuCache eviction (#3541)
- Modify error string to remove Ethereum (#3540)
- Reduce memory needs of `graph::data::Object` (#3557)
- Mark new subgraphs to index only a prefix of bytea columns (#3575)
- Switch from ipfs object stat to dag stat (#3570)
- Unconditionally link IPFS cat & map (#3585)
- Update the chain integration docs (#3590)
- Replace ipfs dag/stat with files/stat (#3597)
- Use connection pool for Elasticsearch (#3591)
- Prepare for private data sources table (#3569)
- Update eth firehose filters (#3596)
- Replace H256 by BlockHash in the Chain Store (#3623)
- Return results for Cosmos (#3649)
- Document metadata tables (#3664)
- Revamp handling of earliest block (#3673)
- Only create exclusion constraints in debug builds (#3675)
- Improve end-to-end test runner (#3689)
- Promote eth_call payload log to debug (#3696)
- Enable fork base for graph-node Docker image (#3713)
- Update Docker remove script (#3723)
- Set maximum operations per connection for Subscriptions (#3735)
- Make fixture chain agnostic in tests (#3740)
- Go one block at a time if requesting all traces for Ethereum JSON-RPC (#3750)
- Firehose cursor newtype (#3754)
- Updated environment-variables.md doc (#3758)
- Make DS Runtime Optional (#3766)

### Dependency Updates

- Remove unused dependencies from the `store` (#3406)
- Bump semver from 1.0.6 to 1.0.7 (#3401)
- Bump quote from 1.0.16 to 1.0.17 (#3400)
- Bump pretty_assertions from 1.2.0 to 1.2.1 (#3433)
- Bump quote from 1.0.17 to 1.0.18 (#3458)
- Bump anyhow from 1.0.54 to 1.0.57 (#3494)
- Bump uuid from 0.8.2 to 1.0.0 (#3495)
- Remove dead futures 0.1 dep (#3521)
- Update WebAssembly libraries (#3523)
- Bump clap from 2.34.0 to 3.1.15 (#3520)
- Bump semver from 1.0.7 to 1.0.9 (#3519)
- Bump prost version to 0.10 (#3531)
- Bump num-traits from 0.2.14 to 0.2.15 (#3545)
- Bump prost from 0.10.1 to 0.10.3 (#3546)
- Bump serde_yaml from 0.8.23 to 0.8.24 (#3547)
- Bump tonic-build from 0.7.1 to 0.7.2 (#3548)
- Bump openssl from 0.10.38 to 0.10.40 (#3549)
- Bump clap from 3.1.15 to 3.1.18 (#3564)
- Upgrade to latest GraphiQL with Tabs (#3529)
- Bump once_cell from 1.10.0 to 1.12.0 (#3583)
- Bump prometheus from 0.13.0 to 0.13.1 (#3581)
- Bump prost from 0.10.3 to 0.10.4 (#3604)
- Bump petgraph from 0.6.0 to 0.6.2 (#3605)
- Bump indexmap from 1.8.0 to 1.8.2 (#3603)
- Remove H256 dependency for status API (#1e6d7fb3f0babb693232fbaf29f5007ef5324080)
- Bump wasm-instrument (#3642)
- Bump uuid from 1.0.0 to 1.1.2 (#3656)
- Bump parking_lot from 0.12.0 to 0.12.1 (#3633)
- Bump crossbeam-utils from 0.8.5 to 0.8.8 (#3631)
- Bump tokio-stream from 0.1.8 to 0.1.9 (#3632)
- Bump clap from 3.1.18 to 3.2.1 (#3661)
- Bump semver from 1.0.9 to 1.0.10 (#3658)
- Bump thread_local from 1.1.3 to 1.1.4 (#3674)
- Bump ethabi from 17.0.0 to 17.1.0 (#3682)
- Bump quote from 1.0.18 to 1.0.20 (#3679)
- Bump crossbeam-channel from 0.5.4 to 0.5.5 (#3683)
- Bump indexmap from 1.8.2 to 1.9.1 (#3690)
- Bump clap from 3.2.1 to 3.2.8 (#3704)
- Bump either from 1.6.1 to 1.7.0 (#3705)
- Bump once_cell from 1.12.0 to 1.12.1 (#3706)
- Bump semver from 1.0.10 to 1.0.12 (#3707)
- Bump openssl from 0.10.40 to 0.10.41 (#3719)
- Bump once_cell from 1.12.1 to 1.13.0 (#3720)
