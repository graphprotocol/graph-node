...
use graph_tests::helpers::{
    get_unique_ganache_counter, make_ganache_uri, make_ipfs_uri, make_postgres_uri, GraphNodePorts,
};
use graph_tests::node::{run_graph_node, stop_graph_node};
use graph_tests::setup::IntegrationTestSetup;
use std::{thread, time};
...
impl PerformanceTestingEnvrionment {
    ...
    async fn deploy_and_seed(&mut self, test_setup: &IntegrationTestSetup) -> anyhow::Result<()> {
        let output = Command::new("yarn")
            ...
            .current_dir("./performance-tests/subgraph")
            .output()
            .await
    ...
    pub async fn spawn_graph_node(&mut self) -> anyhow::Result<IntegrationTestSetup> {
    ...
    pub async fn stop_graph_node(&mut self) -> anyhow::Result<()> {
